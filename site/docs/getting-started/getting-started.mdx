...
dependencies {
    ...
    // Choose one of the following Ktor client engines depending on your platform
    implementation("io.ktor:ktor-client-cio:3.2.2")         // Ktor's CIO engine
    //implementation("io.ktor:ktor-client-okhttp:3.2.2")    // JVM (Android compatible)
    // implementation("io.ktor:ktor-client-apache:3.2.2")   // Apache HttpClient
    // implementation("io.ktor:ktor-client-java:3.2.2")     // Java 11+ HttpClient
    // implementation("io.ktor:ktor-client-jetty:3.2.2")    // Jetty HttpClient
    // implementation("io.ktor:ktor-client-js:3.2.2")       // Javascript/Node
}
```
...
<dependencies>
    ...
    <!-- Ktor Client Engine (REQUIRED) - Choose one -->
    <dependency>
...
## Ktor Client Engine Selection

Aigentic uses Ktor for HTTP communication, which requires a platform-specific client engine:

For detailed information about engine features, which one to choose and alternatives, see the [Ktor Client Engines documentation](https://ktor.io/docs/client-engines.html).

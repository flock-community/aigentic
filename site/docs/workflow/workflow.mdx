...
```kotlin
// Creates a Workflow3<InputType, FirstOutput, SecondOutput, OutputType>
...
// Creates a Workflow2<InitialInput, ProcessedData, FinalOutput>
// Input flows: InitialInput -> ProcessedData -> FinalOutput
...
Workflows also support attachments (PDFs, images, etc.) that are passed to the **first agent only**. Subsequent agents receive structured output from previous agents:

```kotlin
// With attachments
val pdfAttachment = Attachment.Base64.pdf("base64-encoded-pdf-content")
val imageAttachment = Attachment.Base64.jpeg("base64-encoded-image-content")

val workflowRun = workflow.start(initialInput, pdfAttachment, imageAttachment)
...
:::note
Attachments are only passed to the **first agent** in the workflow.
This allows the first agent to process documents, images, or other media files, while subsequent agents work with the structured data output from previous agents.
:::
...
suspend fun processLegalDocument(apiKey: String) {
    ...
    // Creates a Workflow3<String, RedactedDocument, ComplianceReport, LegalSummary>
    // Input flows: String -> RedactedDocument -> ComplianceReport -> LegalSummary
    val workflow = documentRedactor thenProcess complianceChecker thenProcess summarizer

    // Execute the workflow
    val workflowRun = workflow.start(documentText, Attachment.Base64.pdf("base64-encoded-pdf-content"))
...
4. **Structure attachment workflows**: Design the first agent to extract and structure data from attachments, then pass structured data to subsequent agents

...
object McpServers {
    ...
    fun gitlab(
        token: String? = null,
        name: String = "gitlab-client"
    ) = McpConfig.Stdio(
        command = "npx",
        args = listOf("-y", "@modelcontextprotocol/server-gitlab"),
        env = token?.let { mapOf("GITLAB_TOKEN" to it) } ?: emptyMap(),
        name = name
    )

    fun postgres(
        connectionString: String,
        name: String = "postgres-client"
    ) = McpConfig.Stdio(
        command = "npx",
        args = listOf("-y", "@modelcontextprotocol/server-postgres", connectionString),
        name = name
    )

    fun slack(
        token: String,
        name: String = "slack-client"
    ) = McpConfig.Stdio(
        command = "npx",
        args = listOf("-y", "@modelcontextprotocol/server-slack"),
        env = mapOf("SLACK_TOKEN" to token),
        name = name
    )

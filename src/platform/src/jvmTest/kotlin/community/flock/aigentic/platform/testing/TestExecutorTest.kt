...
class TestExecutorTest : DescribeSpec({
    ...
    describe("TestExecutor") {
        ...
        it("should execute test successfully") {
            ...
            val platformMock = createMockPlatform(
                listOf(
                    Message.ToolCalls(
                        listOf(
                            expectedToolCall,
        ...
        it("should fail when expected tool is not called") {
            ...
            testReport.failures.first().reason shouldBe FailureReason.NotCalled(
                mapOf(
        ...
        it("should fail when expected tool is called with unexpected arguments") {
            ...
            testReport.failures.first().reason shouldBe FailureReason.WrongArguments(
                newsEventTool.name,
        ...
        it("should use context of run to initialize test agent") {
            ...
            testReport.successes.first().state.messages.replayCache shouldContainAll listOf(
                textContextMessage,
                base64ContextMessage
            )
        }
        ...
        it("should no use Message.SystemPrompt of run but of agent instead") {
            ...
            val platformMock = createMockPlatform(
                listOf(
                    Message.SystemPrompt("This is an older version version of the system prompt, the agent has a newer version which should be used instead"),
                    Message.ToolCalls(
                        listOf(
                            expectedToolCall,
                            finishedTaskToolCall,
...
fun createMockModel(responses: List<ToolCall>): Model = mockk {
    coEvery { sendRequest(any(), any()) } returnsMany responses.toModelResponse()
}

fun createMockPlatform(messages: List<Message>): Platform = mockk {
    coEvery { getRuns(any()) } returns listOf(
        RunId("some-run-id") to Run(

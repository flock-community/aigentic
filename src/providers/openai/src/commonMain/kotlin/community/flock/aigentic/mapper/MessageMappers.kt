...
object DomainMapper {
    ...
    private fun ChatRole.mapToSender(): Sender = when (this) {
...
object OpenAIMapper {
    ...
    internal fun Message.toOpenAIMessage(): ChatMessage {
        ...
        return when (this) {
            ...
            is Message.ToolCalls -> ChatMessage(
                role = role,
                content = null as String?,
                toolCalls = mapToolCalls(),
            )

            is Message.ToolResult -> ChatMessage(
                role = role,
    ...
    private fun Message.mapChatTextRole(): ChatRole = when (this) {
    ...
    private fun Message.Text.mapChatTextRole() = when (this.sender) {
        Sender.Aigentic -> ChatRole.User
        Sender.Model -> ChatRole.Assistant
    }

    private fun Message.Image.mapImageTextRole() = when (this.sender) {
        Sender.Aigentic -> ChatRole.User
        Sender.Model -> ChatRole.Assistant
    }

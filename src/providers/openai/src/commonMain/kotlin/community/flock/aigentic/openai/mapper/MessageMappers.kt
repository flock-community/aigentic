...
object DomainMapper {
    internal fun ChatMessage.toMessage(): Message =
        when {
            ...
            isToolCallsMessage() -> {
                Message.ToolCalls(toolCalls.mapToolCalls())
            }
            ...
            isTextMessage() -> {
                ...
                aigenticException("Cannot map OpenAI ChatMessage, unknown type: $this")
            }
        }
    ...
    internal fun Message.toOpenAIMessage(): ChatMessage {
        ...
        return when (this) {
            ...
            is Message.StructuredOutput -> {
                ChatMessage(role, response)
            }

            is Message.ExampleToolMessage -> {
                ChatMessage(role = role, content = text, toolCallId = ToolId(id.toString()))
            }

            is Message.Url -> {
                ChatMessage(role = role, listOf(ImagePart(url)))
            }

            is Message.Base64 -> {
                ChatMessage(role = role, listOf(ImagePart(formatDataUrl())))
            }
